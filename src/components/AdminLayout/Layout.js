...
const Layout = (props) => {
      ...
      const getUsers = useCallback(async () => {
            try {
                  ...
                  let res = { data: [] };
                  if (window.localStorage.getItem("su")) res = await adminBackend.getAdmins();
      ...
      const addNewAdmin = async () => {
            ...
            try {
                  ...
                  if (modal.userNew) {
                        ...
                        let app_users = [...users.appUsers];
                        app_users.push(res.data);
                        setUsers({ ...users, appUsers: app_users });
                        onCancelHandler();
                        return null;
                  }
                  if (modal.adminNew) {
                        const res = await adminBackend.addNewAdmin(formData);
                        let updateAdmins = [...users.admins];
                        updateAdmins.push(res.data);
                        setUsers({ ...users, admins: updateAdmins });
                        onCancelHandler();
                        return null;
                  }
            } catch (error) {
