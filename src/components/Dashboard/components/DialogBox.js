...
const DailogBox = ({ open, edit, start, end, report, setModal, items, subItems }) => {
	...
	const getDateInDDMMYYYY = (date) => {
		...
		let mm = x.getMonth() < 10 ? "0" + x.getMonth() : x.getMonth();
		let yy = x.getFullYear();
		return `${dd}-${mm}-${yy}`
	}

	const downloadXLSX = async () => {
		let data = "Start Date,End Date,Event name,Location,Client,Client Email,Client contact,Tech name";
		data = [data.split(",")];
		const entryKeys = [
			"start_date",
			"end_date",
		...
		data.push(Array.from(entryKeys.map((key) => {
			if (key === "start_date") {
				return getDateInDDMMYYYY(startD);
			} else if (key === "end_date") {
				return getDateInDDMMYYYY(endD);
			}
			return form[key]
		})));
	let ws_name = `${getDateInDDMMYYYY(startD)}_${form.client_company}`;
	let wb = xlsx.utils.book_new();
	let ws = xlsx.utils.aoa_to_sheet(data);
	xlsx.utils.book_append_sheet(wb, ws, ws_name);
	xlsx.writeFile(wb, `${getDateInDDMMYYYY(startD)}_${form.client_company}.xlsx`);
};
			...
			<DialogActions className="justify-content-between pl-4 mt-2 pl-16 mb-2">
				...
				<DialogActions className="justify-content-between pl-4 pl-16 mb-2">
					...
					<div className="mr-auto">
						<Button variant="contained" className="btn-dark mr-2" onClick={addEventDetail}>
							Email
						</Button>
						<Button variant="contained" className="btn-dark" onClick={downloadXLSX}>
							Download
						</Button>
						<Button variant="contained" className="btn-dark" onClick={emailToClient}>
							Email
						</Button>
					</div>
					<IconButton>
