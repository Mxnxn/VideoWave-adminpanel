	...
	const submitHandler = async () => {
		try {
			...
			if (state.is_available) {
				...
				formData.set("is_lost", 0);
			} else {
				formData.set("is_available", 0);
				formData.set("is_lost", 1);
			}
			if (state.notes) formData.set("notes", state.notes);
			const res = await inventoryManagementBackend.updateSerialData(
				formData,
				preData.id
			);
			if (res.code === 200) {
				...
				const thisData = {
					...
					lost_quantity: state.lost_quantity,
					is_lost: state.is_lost ? 1 : 0,
					is_available: state.is_available ? 1 : 0,
					serial_number: preData.serial_number,
					qrcode_path: preData.qrcode_path,
					item_id: preData.item_id,
					id: preData.id,
					notes: state.notes,
				};
				console.log("AFTER:", thisData);
				const serials = data.serials;
				const indx = serials.findIndex((el) => el.id === preData.id);
				serials.splice(indx, 1);
				serials.splice(indx, 0, thisData);
				data.serials = serials;
				setSelected({ ...data });
				setSelectedQRCard((prev) => ({ ...prev, hold: thisData }));
				setState({
					total_quantity: 0,
					available_quantity: 0,
					lost_quantity: 0,
