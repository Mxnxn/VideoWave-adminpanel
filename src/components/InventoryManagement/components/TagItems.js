...
const TagItems = ({ items, setModal, setExistingItems, title, toOpen }) => {
      ...
      const onTapEditHandler = async () => {
            try {
                  await inventoryManagementBackend.updateTag(selected.hold.id);
            } catch (error) {
      ...
      const classes = title === "Types" ? "col pr-0 pl-4 h-100 bg-white rounded shadow mt-2" : "col pr-0 pl-4 h-100 bg-white rounded shadow";
            ...
            <div className={classes}>
                  ...
                  <div className=" d-flex pt-3 flex-wrap item-area " style={{ overflowX: "auto", height: "90%", alignContent: "start" }}>
                        ...
                        <Modal
                              ...
                              onCancelHandler={onCancelHandler}
                        />
                        <DeleteModal error={error} onCancelHandler={onCancelHandler} modal={modals.delete} submitHandler={onTapDeleteHandler} />
                  </div>
            </div>
      );
};
