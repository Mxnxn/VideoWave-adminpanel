...
import { DialogActions,Button,AppBar, Toolbar, Typography, IconButton, Dialog, DialogContent, MenuItem, Select, InputLabel, FormControl, MenuList, TextField } from "@material-ui/core";
...
const AddItemModal = ({onCancelHandler,modal,error,title,state,setState,stuffs}) => {
    ...
    const submitHandler = () => {
        ...
        if(dynamicForm.length === 0){
            return setGeneralError("To add Items there should be atleast 1 serial number added!")
        }
        dynamicForm.forEach((el,index)=>{if(!el.serial_number){el.error = `Item is empty, can be located at row ${index+1}.` }else el.error = false});
        
        setDynamicForm([...dynamicForm])
        let duplicateItems = [];
        let isDuplicateAvailable = false;
        const addedSerials = dynamicForm.map((el)=> { return {serial_number : el.serial_number,id:el.id}});
        ...
        duplicateItems.forEach((el,index)=>{
            dynamicForm[el].error = `Item ${el.serial_number} is repeating, can be located at row ${index+1}!`;
            setDynamicForm([...dynamicForm])
        })
					...
					<DialogContent classes={{ root: "p-16 pb-0 sm:p-24 sm:pb-0" }}>
						...
						<div className="d-flex">
                            ...
                            <div className="alert alert-info w-100 rounded mt-2">Note: Enter it Properly, it will be used later!</div>
                                    ...
                                    <AlertDanger error={itemDetails.error ? "Item name is empty!":false} />
                                    ...
                                    {dynamicForm.map((el,index)=>
                                        ...
                                        <div className="row mt-3 ">
                                            ...
                                            <FormControl fullWidth variant="outlined">
                                                <InputLabel >Class</InputLabel>
                                                    <Select
                                                    label="Class"
                                                    onChange={evt=>setItemDetails({...itemDetails,class:evt.target.value})}
                                                    >
                                                        <MenuItem disabled selected value="">
                                                            <em>None</em>
                                                        </MenuItem>
                                                        {classes.map((el,index)=><MenuItem value={el} key={index}>{el}</MenuItem>)}
                                                    </Select>
                                            </FormControl>   
                                            </div>                                             
                                            <div className="col-sm-12 col-xl-6">
                                                <FormControl fullWidth variant="outlined">
                                                    <InputLabel >Category</InputLabel>
                                                        <Select
                                                        onChange={evt=>setItemDetails({...itemDetails,category:evt.target.value})}
                                                        label="Category"
                                                        >
                                                            <MenuItem disabled selected value="">
                                                            <em>None</em>
                                                            </MenuItem>
                                                            {categories.map((el,index)=><MenuItem value={el} key={index}>{el}</MenuItem>)}
                                                        </Select>
                                                </FormControl>    
                                            </div>                                            
										</div>
                                        ...
                                        <div className="col-sm-12 col-xl-6">
                                            <FormControl fullWidth variant="outlined">
                                                <InputLabel >Type</InputLabel>
                                                    <Select
                                                    onChange={evt=>setItemDetails({...itemDetails,type:evt.target.value})}
                                                    label="Type"
                                                    >
                                                        <MenuItem disabled selected value="">
                                                            <em>None</em>
                                                        </MenuItem>
                                                        {types.map((el,index)=><MenuItem value={el} key={index}>{el}</MenuItem>)}
                                                    </Select>
                                            </FormControl>   
                                            </div> 
                                            <div className="col-sm-12 col-xl-6">
                                                <TextField error={itemDetails.error} label="Name" value={itemDetails.name} variant="outlined" fullWidth  onChange={evt=>setItemDetails({...itemDetails,name:evt.target.value})}/> 
                                            </div>                                                             
										</div>
                                        ...
                                        <div key={index} class="row mt-3 ">
                                            <div className="col-sm-12 col-xl-6">
                                                <CustomeTextField error={serialItem.error} index={index} label="Serial Number" value={serialItem.serial_number} type="text" keyName="serial_number" name="serial_number"  onValueChange={onDaynamicValueChange}/>
                                            </div>
                                            <div className="col-sm-12 col-xl-6">
                                                <CustomeTextField error={serialItem.error} index={index} label="Quantity" type="number" value={serialItem.quantity} type="number" keyName="quantity" name="quantity"  onValueChange={onDaynamicValueChange}/>
